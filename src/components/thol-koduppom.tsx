...
// // Define the TypeScript type for a single submission
...
//   phone: string;
//   message: string;
//   createdAt: string; // Assuming the backend sends this
// }
...
//       try {
//         // 1. Correctly fetches from your external backend server
//         const response = await fetch('http://localhost:5001/api/tktm-submissions');

//         if (!response.ok) {
//           // Creates a clear error message if the backend responds with an error (like 404)
...
//       } catch (err) {
//         // Handles network errors (e.g., backend is offline) or other exceptions
...
//   // This is the final, gracefully-handled page layout
//   return (
...
//       {/* 2. Displays a simple error message ONLY if an error exists */}
//       {error && (
//         <p className="text-center mb-4 text-red-500 font-semibold">{error}</p>
//       )}

//       {/* 3. The main table structure is always visible */}
...
//               // If there is data, map over it and create a row for each submission
...
//                   <td className="px-6 py-4 whitespace-pre-wrap max-w-sm">{submission.message}</td>
//                 </tr>
//               ))
//             ) : (
//               // If there is no data OR an error, show a single "Not Found" row
//               <tr>
