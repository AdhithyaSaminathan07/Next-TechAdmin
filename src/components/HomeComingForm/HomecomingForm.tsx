...
const HomeComingForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    Object.keys(formValues).forEach((key) => {
      if (key === "departments" && Array.isArray(formValues[key])) {
        formValues[key].forEach((value: string) => formData.append(key, value));
      } else {
        formData.append(key, formValues[key]);
      }
    });
  ...
  return (
    <div className="min-h-screen w-full bg-gray-100 py-8 px-4 flex justify-center items-start">
      <form
        onSubmit={handleSubmit}
        className="w-full max-w-4xl bg-white shadow-xl rounded-xl p-6 md:p-10 space-y-8 md:space-y-12"
      >
        <div className="text-center">
        ...
        {/* SECTION A: PERSONAL INFO */}
        <section className="bg-gray-50 p-4 rounded-lg shadow-inner space-y-4 md:space-y-6">
              ...
              {["Yes", "No"].map((val) => (
                <label key={val} className="flex items-center gap-2">
                  <input
                    ...
                    name="academicRequirement"
                    value={val}
                    checked={formValues.academicRequirement === val}
                    onChange={handleChange}
                    required
                  />
                  {val}
